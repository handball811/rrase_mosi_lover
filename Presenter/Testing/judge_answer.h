...
int snake(int k, int y, char *l, char *r, int l_len, int r_len)      //  対角線が存在する場合は、移動する
{
	int x = y - k;
...
int edit_distance(output_structure *l, output_structure *r)
{
	int i,k;
	// 初期の代入
	char *l_ans = l->ans;
	...
	for(i=0; fp[DELTA + offset] != r_len; ++i) {
        for(k = -i; k < DELTA; ++k)
            fp[k + offset] = snake(k, MAX(fp[k-1+offset] + 1, fp[k+1+offset]), l_ans, r_ans, l_len, r_len);
        for(k = DELTA + i; k > DELTA; --k)
            fp[k + offset] = snake(k, MAX(fp[k-1+offset] + 1, fp[k+1+offset]), l_ans, r_ans, l_len, r_len);
        fp[DELTA + offset] = snake(k, MAX(fp[DELTA-1+offset] + 1, fp[DELTA+1+offset]), l_ans, r_ans, l_len, r_len);
    }
    return DELTA + (i - 1) * 2;
}
#endif
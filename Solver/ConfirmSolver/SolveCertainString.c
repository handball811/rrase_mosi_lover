...
int InsertWord(char* str, int strLen, char* target, int targetLen, int phase) {
    int i = targetLen - 1;

    while(i < strLen) { // 1文字ずつずらしていく。
        int matchCharaNum = 0;

        for(int j = 0; j < targetLen;
            j++) { // 部分文字列全体を見て、何文字完全一致しているかの確認。
            if(target[targetLen - j - 1] == str[i - j]) // 完全一致。
                matchCharaNum++;
            else if(str[i - j] !=
                    'x') { // 完全一致でもxでもないなら100%当てはまらない。
                matchCharaNum = 0;
                break;
            }
        }
    ...
    for(int phase = 0; phase < PHASE_NUM; phase++) {
        ...
        while(1) {
            ...
            for(int i = 0; i < inputStructure->partsNum; i++) {
                ...
                if(InsertWord(middleStructure->str, inputStructure->strLen,
                              inputStructure->parts[i],
                              strlen(inputStructure->parts[i]), phase)) { // 入れてみる
                    flg[i] = 1; // 入ったらフラグを立てる。
                    insertNum++;
                }
